...
- Issue 67 fix in progress: added explicit same-session memory lookup routing for tasks that request stored session/project memory, so `latest_approved_output` is read from `project_memory` instead of being confused with current prompt text
